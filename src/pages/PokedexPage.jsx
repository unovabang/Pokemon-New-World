...
/* Legacy : conservé pour la compatibilité (modal, table) — refait via TYPE_META. */
const TYPE_COLORS = Object.fromEntries(
  Object.entries(TYPE_META).map(([k, m]) => [
    k,
    { bg: m.from, border: m.to, text: m.fg },
  ])
);
...
/* Pill réutilisable avec icône + label. */
...
/** Menu déroulant personnalisé avec options colorées par type + icône FA */
              ...
              {filtered.map((pokemon, i) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    className={`pokedex-card${pokemon.availability === "new" ? " pokedex-card--available-new" : ""}${pokemon.availability === "unavailable" ? " pokedex-card--unavailable" : ""}${meta ? " pokedex-card--typed" : ""}`}
